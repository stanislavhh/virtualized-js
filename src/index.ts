...
export default class VirtualizedList<I> implements IVirtualizedList<I> {
   ...
   private get fullHeight() {
    ...
    init() {
        this.renderSubjects.forEach((subject, i) => 
            subject.subscribe({ name: 'render' + i, update: this.render })) 
    ...
    private isItemAlreadyRendered(index: number) {
        return !this.isRenderedFirstTime()
    ...
    private mountItems() {
        ...
        for (let i = index; i < visibleItemsLength + index; i++) {
           ...
           if (itemNode) {
            ...
            fullHeightWindow?.[
                !this.isRenderedFirstTime() && i < (this.getPrevIndex()) ? 'prepend': 'appendChild'
            ](itemNode)
        }
    }
